...
export const revertEventChallenge = async (challengeId) => {
  ...
  // Per ogni squadra che aveva preso punti, li togliamo (usando set + Math.max per sicurezza assoluta)
  for (const res of challengeData.finalResults) {
      if (res.earnedPoints > 0 && res.teamId) {
          ...
          if(teamSnap.exists()){
              ...
              const newScore = Math.max(0, currentScore - res.earnedPoints);
...
export const addManualPointsToEventTeams = async (teamPoints) => {
  ...
  for (const [teamId, points] of Object.entries(teamPoints)) {
    ...
    if (numPoints !== 0) {
      const teamRef = doc(db, 'event_teams', teamId);
      // Se sono negativi, ci assicuriamo che non vadano sotto lo zero
      if (numPoints < 0) {
         const teamSnap = await getDoc(teamRef);
         if(teamSnap.exists()){
             const currentScore = teamSnap.data().score || 0;
             batch.update(teamRef, { score: Math.max(0, currentScore + numPoints) });
         }
      } else {
...
export const startEventPerformance = async (performanceId) => {
  ...
  // 1. Reset di eventuali altre esibizioni 'active'
...
// 🔴 ANNULLA un'esibizione LIVE — chiude il popup e rimette in coda
...
export const completeEventPerformance = async (performanceId) => {
  ...
  const totalSum = votes.reduce((a, b) => a + Number(b), 0); // SOMMA
...
// ==========================================
// 🌟 PROPAGA SOMMA AL FANTA (SICURO E FIXATO)
// ==========================================
...
export const rollbackFantaPropagation = async (performanceId) => {
  ...
  // 1. Trova TUTTE le richieste create da questa performance
  const qReq = query(collection(db, 'requests'), where('challengeId', '==', `televoto_${performanceId}`));
  const reqSnap = await getDocs(qReq);

  // 2. Per ogni richiesta trovata, leggiamo l'utente e gli sottraiamo i punti
  for (const rDoc of reqSnap.docs) {
    ...
    if (rData.matricolaId && rData.puntiRichiesti) {
      ...
      if (userSnap.exists()) {
         ...
         const newPts = Math.max(0, currentPts - rData.puntiRichiesti); 
         batch.update(userRef, { punti: newPts });
      }
...
// 🏆 ASSEGNAZIONE FINALE PUNTI ALLE SQUADRE (Corretta per Regola 7 e 8)
export const assignAllPerformancePoints = async () => {
  ...
  // Creiamo un accumulatore separato: 
  // flatPoints = Somma netta dell'esibizione di Squadra
  // indSum / indCount = Dati per fare la media delle esibizioni individuali
  const teamUpdates = {};
  teams.forEach(t => {
  ...
  perfSnap.docs.forEach(pDoc => {
    ...
    if (perf.isTeam && perf.teamId) {
      // REGOLA 7: Tema di squadra -> Si somma INTERO (non fa media)
      if (teamUpdates[perf.teamId]) {
          teamUpdates[perf.teamId].flatPoints += perf.totalScore;
      }
    } else if (perf.matricolaId) {
      // REGOLA 8: Tema individuale -> Finisce nel calderone per fare la MEDIA
  ...
  for (const [teamId, data] of Object.entries(teamUpdates)) {
    // Punteggio base = Tema di squadra (es. 180)
    let finalPointsToAdd = data.flatPoints; 
    
    // Aggiungiamo la media dei temi individuali (es. 150)
    if (data.indCount > 0) {
...
// ==========================================
// 🔗 PROPAGAZIONE AL FANTAMATRICOLA (DA SFIDE)
// ==========================================
...
export const propagateChallengeToFanta = async (challengeId) => {
  ...
  const prepareMatricolaUpdate = async (userId, points, isWinner) => {
      ...
      batch.update(userRef, { punti: Math.max(0, currentPoints + points) });
...
export const revertFantaPropagation = async (challengeId) => {
  ...
  for (const reqDoc of reqSnap.docs) {
    ...
    if (data.manual && data.challengeTitle?.startsWith('Evento -')) {
      ...
      if(userSnap.exists()){
          const currentPts = userSnap.data().punti || 0;
          const newPts = Math.max(0, currentPts - data.puntiRichiesti);
          batch.update(userRef, { punti: newPts });
      }
