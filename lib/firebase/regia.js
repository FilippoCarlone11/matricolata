  ...
  for (const rDoc of reqSnap.docs) {
    ...
    if (rData.matricolaId && rData.puntiRichiesti) {
      ...
      if (userSnap.exists()) {
         ...
         const newEveningPts = Math.max(0, currentEveningPts - rData.puntiRichiesti);
...
export const propagateChallengeToFanta = async (challengeId) => {
  ...
  const prepareMatricolaUpdate = async (userId, points, isWinner) => {
      ...
      batch.update(userRef, { 
          punti: Math.max(0, currentPoints + points),
          puntiSerata: Math.max(0, currentEveningPoints + points) 
      });
  ...
  for (const reqDoc of reqSnap.docs) {
    ...
    if (data.manual && data.challengeTitle?.startsWith('Evento -')) {
      ...
      if(userSnap.exists()){
          ...
          const newEveningPts = Math.max(0, currentEveningPts - data.puntiRichiesti); 
