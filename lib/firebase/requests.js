import { db } from './init';
import { collection, doc, getDoc, updateDoc, addDoc, deleteDoc, query, where, getDocs, onSnapshot, serverTimestamp, runTransaction } from 'firebase/firestore';
...
export const approveRequest = async (requestId, matricolaId, points) => {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    transaction.update(requestRef, {
        ...
        // Salviamo i punti EFFETTIVAMENTE assegnati: se l'admin approva con un
        // valore diverso da quello richiesto, la revoca deve togliere ESATTAMENTE
        // questo importo (altrimenti i punti si disallineano).
...
// Super-admin: nasconde/mostra un post nel feed globale.
// Non elimina la richiesta né altera i punti: cambia solo la visibilità.
export const setPostFeedVisibility = async (requestId, hidden) => {
  const requestRef = doc(db, 'requests', requestId);
  await updateDoc(requestRef, { feedHidden: !!hidden });
};

// Super-admin: ELIMINA DEFINITIVAMENTE la richiesta/post (irreversibile).
// ATTENZIONE: non rimuove i punti già assegnati all'utente (per quello esiste
// la revoca); cancella solo il documento della richiesta dal feed/storico.
