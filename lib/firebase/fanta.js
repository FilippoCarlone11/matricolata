...
export const approveRequest = async (requestId, matricolaId, points) => {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // ✅ SALVIAMO isEveningEvent NELLA REQUEST! Così la revoca sarà indipendente dal catalogo sfide
...
export const revokeApprovedRequest = async (requestId, userId, clientPoints) => {
  await runTransaction(db, async (transaction) => {
    ...
    // ✅ PRENDIAMO I PUNTI DAL DB, IGNORANDO IL CLIENT (Evita bug di sincronia)
    const puntiDaRimuovere = Number(reqData.puntiRichiesti) || Number(clientPoints) || 0;

    // ✅ CONTROLLIAMO SE ERA SERALE DIRETTAMENTE DALLA RICHIESTA (o fallback sulla sfida per la retrocompatibilità)
    ...
    // ✅ SOTTRAZIONE SICURA (Se è un malus negativo es. -50, fare "current - (-50)" aggiungerà i punti correttamente)
