...
export const revokeApprovedRequest = async (requestId, userId, points) => {
  await runTransaction(db, async (transaction) => {
    ...
    const userUpdates = { punti: Math.max(0, (userDoc.data().punti || 0) - points) };
    if (isEvening) {
        userUpdates.puntiSerata = Math.max(0, (userDoc.data().puntiSerata || 0) - points);
    }
    transaction.update(userRef, userUpdates);
  });
};
...
export const assignExistingChallenge = async (userId, challengeId, points, title) => {
  await runTransaction(db, async (transaction) => {
    ...
    const userUpdates = { punti: Math.max(0, currentPoints + points) };
    if (isEvening) {
        userUpdates.puntiSerata = Math.max(0, currentEveningPoints + points);
    }
