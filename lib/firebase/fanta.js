...
export const assignExistingChallenge = async (userId, challengeId, points, title) => {
  await runTransaction(db, async (transaction) => {
    ...
    // Controlla se il nome contiene parole alcoliche
    const t = (title || "").toLowerCase();
    if (t.includes('fegato') || t.includes('drink') || t.includes('cocktail') || t.includes('shot')) {
        isDrink = true;
    }
    ...
    if (challengeId && challengeId.length > 15) {
        ...
        if (challengeDoc.exists()) {
            if (challengeDoc.data().isEveningEvent) isEvening = true;
            const ct = (challengeDoc.data().titolo || "").toLowerCase();
            if (ct.includes('fegato') || ct.includes('drink')) isDrink = true;
        }
    }
    ...
    const currentPoints = Number(userDoc.data().punti) || 0;
    const currentEveningPoints = Number(userDoc.data().puntiSerata) || 0;
    const currentDrinks = Number(userDoc.data().drinkCount) || 0; // Prende i drink attuali
    
    const userUpdates = { punti: currentPoints + puntiDaAggiungere };
    if (isEvening) {
        userUpdates.puntiSerata = currentEveningPoints + puntiDaAggiungere;
    }
    if (isDrink && puntiDaAggiungere > 0) {
        userUpdates.drinkCount = currentDrinks + 1; // AGGIUNGE IL DRINK!
    }
    ...
    transaction.set(requestRef, {
      ...
      manual: true,
      isEveningEvent: isEvening,
      isDrinkEvent: isDrink, // Salva per lo storico
      approvedAt: serverTimestamp(),
      createdAt: serverTimestamp()
    });
  });
