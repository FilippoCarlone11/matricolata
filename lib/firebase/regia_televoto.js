import { db } from './init';
import { collection, doc, getDoc, updateDoc, addDoc, deleteDoc, query, where, getDocs, serverTimestamp, writeBatch, increment, runTransaction } from 'firebase/firestore';
...
export const submitLiveVote = async (userId, voteValue) => {
  // Normalizziamo il voto a un intero 1-10: stesso vincolo imposto dalle
  // Security Rules, così un voto malformato viene bloccato già qui.
  const v = Math.round(Number(voteValue));
  if (!Number.isInteger(v) || v < 1 || v > 10) {
    throw new Error("Voto non valido: deve essere un numero intero da 1 a 10.");
  }
  const liveRef = doc(db, 'live_voting', 'current');
...
export const propagateSinglePerformanceToFanta = async (performanceId) => {
  // Transazione atomica: il guard `fantaPropagated` viene letto e scritto
  // nella stessa transazione, quindi un doppio click non può propagare i
  // punti due volte (prima era getDoc + writeBatch, soggetto a TOCTOU).
  await runTransaction(db, async (tx) => {
    ...
    if (perfData.isTeam) {
      ...
      const teamSnap = await tx.get(doc(db, 'event_teams', perfData.teamId));
      ...
      // 1) TUTTE le letture prima di qualsiasi scrittura (vincolo transazioni)
      const memberSnaps = [];
      for (const memberId of members) {
        const userRef = doc(db, 'users', memberId);
        memberSnaps.push({ memberId, userRef, snap: await tx.get(userRef) });
      }

      // 2) Scritture
      for (const { memberId, userRef, snap } of memberSnaps) {
        if (snap.exists()) {
          const currentPoints = Number(snap.data().punti) || 0;
          const currentEveningPoints = Number(snap.data().puntiSerata) || 0;
          tx.update(userRef, {
        ...
        tx.set(doc(collection(db, 'requests')), {
    ...
    tx.update(perfRef, { fantaPropagated: true });
  });
};
