...
export const manualAddPoints = async (userId, points, reason) => {
  await runTransaction(db, async (transaction) => {
    ...
    transaction.set(requestRef, {
      matricolaId: userId, challengeId: reason || 'Bonus Manuale', puntiRichiesti: points,
      status: 'approved', manual: true, approvedAt: serverTimestamp(), createdAt: serverTimestamp()
    });
  });
};

// NUOVO: Assegnazione Bonus Esistente (Admin sceglie da lista)
export const assignExistingChallenge = async (userId, challengeId, points, title) => {
    await runTransaction(db, async (transaction) => {
      ...
      transaction.set(requestRef, {
        matricolaId: userId, 
        challengeId: challengeId, // Salviamo ID reale per riferimento
        puntiRichiesti: points,
        status: 'approved', 
        manual: true, // Lo segniamo manuale perché inserito da admin
