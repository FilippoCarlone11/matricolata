import { 
  ...
  query, where, getDocs, onSnapshot, serverTimestamp, orderBy, runTransaction, limit, writeBatch, arrayRemove
...
export const deleteUserDocument = async (userId) => {
  try {
    ...
    // 1. TROVA E CANCELLA TUTTE LE RICHIESTE DELL'UTENTE
    // Cerchiamo nella collection 'requests' dove userId == userId da eliminare
    const requestsRef = collection(db, 'requests');
    const qRequests = query(requestsRef, where('userId', '==', userId));
    ...
    // Cerchiamo tutti gli utenti che hanno questo userId nel loro array mySquad
    const usersRef = collection(db, 'users');
    ...
    squadSnap.forEach((doc) => {
      ...
      const updates = {
        mySquad: arrayRemove(userId) // Rimuove l'ID dall'array
      };

      // Se la matricola eliminata era il capitano, resettiamo il capitano
    ...
    // 3. ELIMINA IL DOCUMENTO DELL'UTENTE STESSO
    const userDocRef = doc(db, 'users', userId);
    batch.delete(userDocRef);

    // ESEGUI TUTTE LE OPERAZIONI INSIEME
    await batch.commit();
    console.log(`Utente ${userId} eliminato con successo (Cascade completato).`);
    return true;

  } catch (error) {
    console.error("Errore durante la cancellazione a cascata:", error);
    throw error;
  }
};
...
// Unica fonte di verità: config/appSettings
export const getSystemSettings = async () => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      // Se non esiste, inizializza con i default
      const defaults = { 
    ...
    return { registrationsOpen: true, cacheEnabled: true, cacheDuration: 30 };
  }
};
