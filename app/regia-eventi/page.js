...
import {
    auth, db, getAllUsers,
    createEventTeam, deleteEventTeam, updateEventTeamName,
...
import { Settings, ShieldAlert, Loader2, LogOut, Plus, Trash2, Users, UserMinus, Swords, CheckCircle2, Trophy, Wrench, MonitorPlay, ArrowLeft, Minus, ChevronRight, RotateCcw, Edit2, Check, RadioTower, Play, EyeOff, Radio, Send } from 'lucide-react';
...
export default function PuntiDashboard() {
    ...
    // --- NUOVO STATO LOADING PROPAGAZIONE ---
            ...
            <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-4 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
                    ...
                    <a href="https://matricolata.it" className="flex-1 bg-[#B41F35] text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-[#90192a] transition-all flex items-center justify-center">
                        <ArrowLeft size={18} className="mr-1"/> Torna all'App
                    </a>
                </div>
            </div>
        );
                                    ...
                                    {eventPerformances.filter(p => p.status === 'pending').map(perf => (
                                        <div key={perf.id} className="bg-gray-800 border border-gray-700 rounded-2xl p-4 flex flex-col justify-between shadow-lg group hover:border-green-500/50 transition-colors">
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    <h3 className="font-bold text-lg text-white group-hover:text-green-400 transition-colors">{perf.matricolaName}</h3>
