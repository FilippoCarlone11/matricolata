...
// RIMOSSO: import { getLeaderboards, getFullSquadDetails } from '@/lib/firebase';
import { Trophy, User, Users, Shield, X, Crown } from 'lucide-react';

// AGGIUNTO PROP: preloadedUsers
export default function Classifiche({ preloadedUsers = [] }) {
  ...
  // Rimosso loadingTeam perché ora è istantaneo
  ...
  const calculateLeaderboards = (users) => {
      // 1. Calcolo Classifica Matricole
      const m = users.filter(u => u.role === 'matricola').sort((a, b) => (b.punti || 0) - (a.punti || 0));
      
      // 2. Calcolo Classifica Fantallenatori (con logica Capitano)
      const f = users
  ...
  const handleTeamClick = (user) => {
    if (view !== 'fanta' || !user.mySquad || user.mySquad.length === 0) return;
    
    setSelectedTeam(user);
    // Recuperiamo i dettagli filtrando l'array che abbiamo già in memoria! (0 letture)
    const details = preloadedUsers.filter(u => user.mySquad.includes(u.id));
  ...
  return (
    <div>
      ...
      <List items={view === 'matricole' ? matricole : fantallenatori} isFanta={view === 'fanta'} />
