...
export default function ShareStoryModal({ item, onClose, t }) {
  ...
  useEffect(() => {
    ...
    const draw = async () => {
      ...
      ctx.clip();
      if (avatar) {
        // copre il cerchio mantenendo le proporzioni
        const s = Math.max((2 * r) / avatar.img.width, (2 * r) / avatar.img.height);
        const dw = avatar.img.width * s;
        const dh = avatar.img.height * s;
        ctx.drawImage(avatar.img, cx - dw / 2, cy - dh / 2, dw, dh);
      } else {
        // fallback: iniziali su sfondo brand
      ...
      roundRect(ctx, pillX, pillY, pillW, 78, 39);
      ctx.fill();
