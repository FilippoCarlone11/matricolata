...
import { Search, Calendar, Trash2, PlusCircle, ArrowLeft, Frown, Zap, EyeOff, Users, Trophy } from 'lucide-react';
...
export default function AdminMatricolaHistory() {
  ...
  // Funzione per aggiornare i punti totali visualizzati dopo una modifica
  const refreshSelectedUser = async () => {
    if (!selectedUser) return;
    const freshData = await getUserData(selectedUser.id);
    setSelectedUser(freshData); // Aggiorna l'UI con i nuovi punti
    await loadUserHistory(selectedUser.id); // Ricarica lo storico
  };
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="relative">
          <input 
            type="text" placeholder="Scrivi il nome..." 
